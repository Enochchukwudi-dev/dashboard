...
const AppSidebar: React.FC = () => {
  const { isExpanded, isMobileOpen, isHovered, setIsHovered, toggleMobileSidebar, setActiveItem } = useSidebar();
        ...
        <li key={nav.name}>
          {nav.subItems ? (
            ...
            nav.path && (
              <Link
                href={nav.path}
                onClick={() => { setActiveItem(nav.path ?? nav.name); if (isMobileOpen) toggleMobileSidebar(); }}
                className={`menu-item group ${
              ...
              <ul className="mt-2 space-y-1 ml-9">
                {nav.subItems.map((subItem) => (
                  <li key={subItem.name}>
                    <Link
                      href={subItem.path}
                      onClick={() => { setActiveItem(subItem.path ?? subItem.name); if (isMobileOpen) toggleMobileSidebar(); }}
  ...
  return (
    <>
      ...
      >
        <Link href="/" onClick={() => { if (isMobileOpen) toggleMobileSidebar(); }} className="flex items-center gap-2">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              <Image src="/images/logo/lago.png" alt="Proofly logo" width={28} height={28} />
              <span className="text-2xl font-semibold text-gray-900 dark:text-white">Proofly</span>
            </>
          ) : (
            <>
              <Image src="/images/logo/lago.png" alt="Proofly logo" width={20} height={20} />
    ...
    {isMobileOpen && (
      <div
        className="fixed inset-0 bg-white/2 dark:bg-black/30 backdrop-blur-sm z-40 lg:hidden"
