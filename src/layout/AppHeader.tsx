...
const AppHeader: React.FC = () => {
  // Keep the application (user) menu open by default on mobile
  const [isApplicationMenuOpen, setApplicationMenuOpen] = useState(true);

  const { isMobileOpen, toggleSidebar, toggleMobileSidebar } = useSidebar();

  const showOnMobile = isApplicationMenuOpen && !isMobileOpen;
  ...
  return (
    <header className="sticky top-0 flex w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="relative flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        ...
        <div
          className={`${showOnMobile ? "flex" : "hidden"} items-center justify-between w-full gap-4 px-5 py-4 lg:flex lg:justify-end lg:px-0 lg:shadow-none`}
        >
